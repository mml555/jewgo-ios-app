...
class CaptchaService {
  ...
  async verifyCaptcha(
    ...
    try {
      ...
      await this.logCaptchaChallenge(
        flow,
        providerName,
        result.success,
        result.score,
      ...
      logger.error(
        `CAPTCHA verification failed for provider ${providerName}:`,
        error,
      );
  ...
  async verifyWithRiskAssessment(
    flow,
    token,
    providerName,
    userId,
    ...
    const shouldTrigger = await this.shouldTriggerCaptcha(
      flow,
      userId,
      ipAddress,
      userAgent,
    ...
    // Verify CAPTCHA
    return await this.verifyCaptcha(
      providerName,
      token,
      flow,
      ipAddress,
  ...
  async logCaptchaChallenge(
    ...
    try {
      ...
      logger.error('Failed to log CAPTCHA challenge:', error);
    }
  }
