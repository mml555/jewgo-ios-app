...
class GuestService {
  ...
  async revokeGuestSession(sessionToken) {
    try {
      ...
      logger.error('Error revoking guest session:', error);
      return false;
    }
  }
  ...
  async getGuestPermissions() {
    try {
      ...
      logger.error('Error getting guest permissions:', error);
      return [];
    }
  }
  ...
  async hasGuestPermission(permission, resource = null) {
    try {
      ...
      logger.error('Error checking guest permission:', error);
      return false;
    }
  }
  ...
  async cleanupExpiredSessions() {
    try {
      const result = await this.db.query(
        'SELECT deleted_count FROM cleanup_expired_guest_sessions()',
      );
  ...
  async getGuestSessionStats() {
    try {
      ...
      logger.error('Error getting guest session stats:', error);
      return null;
    }
  }
  ...
  async logGuestEvent(sessionId, event, success, details = {}) {
    try {
      ...
      logger.error('Failed to log guest event:', error);
    }
  }
  ...
  async convertGuestToUser(sessionToken, userData) {
    ...
    try {
      ...
      logger.error('Error converting guest to user:', error);
      throw error;
    } finally {
      client.release();
    }
