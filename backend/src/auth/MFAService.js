...
class MFAService {
  ...
  async verifyTOTP(userId, token) {
    ...
    try {
      ...
      if (verified) {
        ...
        await this.logMFAEvent(userId, 'totp_verify', false, {
          method: 'totp',
  ...
  async authenticateWebAuthn(userId, credentialData) {
    ...
    try {
      ...
      if (isValid) {
        ...
        `,
          [
            credentialData.counter || parseInt(meta.counter, 10) + 1,
            userId,
  ...
  async generateRecoveryCodes(userId) {
    ...
    await this.db.query(
      ...
      VALUES ${hashedCodes
        .map(
          (_, index) =>
            `($1, $${index + 2}, 'mfa_recovery', NOW() + INTERVAL '1 year')`,
        )
        .join(', ')}
    `,
      [userId, ...hashedCodes],
  ...
  async verifyRecoveryCode(userId, code) {
    ...
    if (result.rows.length > 0) {
      ...
      await this.logMFAEvent(userId, 'recovery_code_used', true, {
