...
class ShtetlStoreController {
  ...
  static async getAllStores(req, res) {
    try {
      ...
      // Add ordering
      const validSortColumns = ['name', 'rating', 'created_at', 'updated_at', 'product_count'];
      const sortColumn = validSortColumns.includes(sortBy) ? `s.${sortBy}` : 's.created_at';
      ...
      console.error('Error fetching shtetl stores:', error);
      res.status(500).json({
        success: false,
  ...
  static async createStore(req, res) {
    try {
      ...
      const values = [
        name, description, ownerId, address, city, state, zipCode,
        phone, email, website, storeType, kosherLevel,
        deliveryAvailable, pickupAvailable, shippingAvailable
      ];
  ...
  static async addStoreReview(req, res) {
    try {
      ...
      const existingReviewQuery = 'SELECT id FROM shtetl_store_reviews WHERE store_id = $1 AND user_id = $2';
      const existingReview = await pool.query(existingReviewQuery, [id, userId]);
      ...
      const result = await pool.query(query, [id, userId, rating, title, content]);
