-- Performance optimizations for specials (corrected version with proper column names)
-- This creates materialized views and triggers for enhanced performance

BEGIN;

-- Create materialized view for active specials (with correct column names)
...
-- Create materialized view for restaurants with specials
...
BEGIN
    ...
    SELECT 
        ...
        COUNT(*) FILTER (WHERE is_active AND valid_from <= NOW() AND valid_until >= NOW()) as active_specials,
        ...
        (SELECT s.id FROM specials s WHERE s.claims_total = (SELECT MAX(claims_total) FROM specials WHERE claims_total > 0) LIMIT 1) as top_performing_special_id,
        (SELECT s.title FROM specials s WHERE s.claims_total = (SELECT MAX(claims_total) FROM specials WHERE claims_total > 0) LIMIT 1) as top_performing_special_title,
        (SELECT e.name FROM specials s JOIN entities e ON s.business_id = e.id WHERE s.claims_total = (SELECT MAX(claims_total) FROM specials WHERE claims_total > 0) LIMIT 1) as top_performing_business_name,
        (SELECT MAX(claims_total) FROM specials WHERE claims_total > 0) as top_performing_claims_total
    FROM specials
...
BEGIN
    ...
    SELECT 
        ...
        COUNT(s.id) as active_specials_count,
        MAX(s.priority) as max_priority,
        (SELECT s2.id FROM specials s2 WHERE s2.business_id = e.id AND s2.is_active = true ORDER BY s2.priority DESC, s2.valid_until ASC LIMIT 1) as top_special_id,
        (SELECT s2.title FROM specials s2 WHERE s2.business_id = e.id AND s2.is_active = true ORDER BY s2.priority DESC, s2.valid_until ASC LIMIT 1) as top_special_title,
        (SELECT s2.discount_label FROM specials s2 WHERE s2.business_id = e.id AND s2.is_active = true ORDER BY s2.priority DESC, s2.valid_until ASC LIMIT 1) as top_special_discount_label,
        (SELECT s2.claims_total FROM specials s2 WHERE s2.business_id = e.id AND s2.is_active = true ORDER BY s2.priority DESC, s2.valid_until ASC LIMIT 1) as top_special_claims_total,
        (SELECT s2.max_claims_total FROM specials s2 WHERE s2.business_id = e.id AND s2.is_active = true ORDER BY s2.priority DESC, s2.valid_until ASC LIMIT 1) as top_special_max_claims_total,
        (SELECT s2.valid_until FROM specials s2 WHERE s2.business_id = e.id AND s2.is_active = true ORDER BY s2.priority DESC, s2.valid_until ASC LIMIT 1) as top_special_valid_until
    FROM entities e
    JOIN specials s ON s.business_id = e.id
    WHERE e.entity_type = 'restaurant'
      AND s.is_active = true
