...
import { hapticButtonPress, hapticSuccess, hapticWarning } from '../utils/hapticFeedback';
import { useStableCallback, useDebouncedCallback } from '../utils/performanceOptimization';
import {
...
const BusinessHoursSelector: React.FC<BusinessHoursSelectorProps> = memo(({
  ...
  const [validationResult, setValidationResult] = useState(() => validateBusinessHours(hours));
  ...
  const debouncedValidation = useDebouncedCallback((hoursData: BusinessHoursData) => {
    if (enableRealTimeValidation) {
      const result = validateBusinessHours(hoursData);
      setValidationResult(result);
    }
  }, 300, [enableRealTimeValidation]);
    ...
    if (enableRealTimeValidation) {
      ...
      if (isScreenReaderEnabled) {
        if (hasErrors) {
          ...
          announceForScreenReader(
            `Business hours validation: ${errorCount} error${errorCount !== 1 ? 's' : ''} found`,
            'assertive'
          );
        } else if (hasWarnings) {
          const warningCount = Object.keys(validationResult.warnings).length;
          announceForScreenReader(
            `Business hours validation: ${warningCount} warning${warningCount !== 1 ? 's' : ''} found`,
            'polite'
          );
  ...
  }, [validationResult, isScreenReaderEnabled, enableRealTimeValidation, validationScale]);

  // Update a specific day's hours
  const updateDayHours = useStableCallback((day: string, updates: Partial<DayHours>) => {
    const updatedHours = {
      ...currentHours,
      [day]: {
        ...currentHours[day],
  ...
  const handleCopyHours = useStableCallback((sourceDay: string) => {
    ...
    Alert.alert(
      'Copy Hours',
      `Copy ${sourceDay}'s hours (${utilFormatTimeForDisplay(sourceDayHours.openTime)} - ${utilFormatTimeForDisplay(sourceDayHours.closeTime)}) to which days?`,
      [
        {
          text: 'All Other Days',
        ...
        {
          text: 'Weekdays Only',
          onPress: () => copyToMultipleDays(sourceDay, weekdays.filter(day => day !== sourceDay)),
        },
        {
          text: 'Weekends Only',
          onPress: () => copyToMultipleDays(sourceDay, weekends.filter(day => day !== sourceDay)),
        },
        {
          text: 'Cancel',
          style: 'cancel',
  ...
  const copyToMultipleDays = useStableCallback((sourceDay: string, targetDays: string[]) => {
    const sourceDayHours = currentHours[sourceDay];
  ...
  const handleSetAllWeekdays = useStableCallback(() => {
    ...
    if (isScreenReaderEnabled) {
      announceForScreenReader(
        'Set all weekdays to 9 AM to 5 PM',
        'polite'
      );
    }
  ...
  const handleSetAllWeekends = useStableCallback(() => {
    ...
    if (isScreenReaderEnabled) {
      announceForScreenReader(
        'Set all weekends to 10 AM to 6 PM',
        'polite'
      );
    }
  }, [currentHours, onHoursChange, hapticFeedback, isScreenReaderEnabled]);
    ...
    >
      ...
      <View style={styles.header}>
        ...
        >
          ...
          <TouchableOpacity
            style={[styles.bulkButton, styles.bulkButtonDanger, loading && styles.bulkButtonDisabled]}
            ...
            disabled={loading}
          >
            <Text style={[styles.bulkButtonText, styles.bulkButtonTextDanger]}>Close All</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      >
        {dayNames.map((day) => {
          ...
          if (validationResult.errors[day]) dayErrors.push(validationResult.errors[day]);
      ...
      <Animated.View 
        ...
        accessibilityRole="group"
        accessibilityLabel={`Business hours validation summary. ${validationSummary.openDaysCount} of 7 days are open. ${
          validationSummary.hasErrors ? 'Has errors that need to be fixed.' : 
          validationSummary.hasWarnings ? 'Has warnings to review.' :
          validationSummary.isValid ? 'All hours are valid.' : ''
        }`}
      ...
      >
        <View style={styles.summaryRow}>
          ...
          <Text 
            style={[
              styles.summaryValue,
              validationSummary.openDaysCount === 0 && styles.summaryValueError
            ]}
            accessibilityRole="text"
        ...
        {validationSummary.hasWarnings && (
          ...
          >
            {Object.entries(validationSummary.warnings).map(([day, warning]) => (
              <Text 
                key={day} 
