...
const kosherCategories: KosherCategory[] = [
  ...
  },
  {
    ...
    icon: '🧀',
    requirements: [
      'Cholov Yisroel preferred',
      'Proper kashrut supervision',
      'No mixing with meat',
    ],
  },
  {
...
const EnhancedKosherSelector: React.FC<EnhancedKosherSelectorProps> = memo(
  ({
    ...
    const [showRequirements, setShowRequirements] = useState<string | null>(
      null,
    );
    ...
    const handleRequirementsToggle = useCallback(
      (categoryId: string) => {
        ...
        hapticButtonPress();
        setShowRequirements(
          showRequirements === categoryId ? null : categoryId,
        );
      },
    ...
    const selectedCategory = useMemo(
      () => kosherCategories.find(cat => cat.id === category),
      [category],
    );

    // Get selected agency
    const selectedAgency = useMemo(
      () => certifyingAgencies.find(ag => ag.name === agency),
      [agency],
    );

    // Get category option styles
    const getCategoryOptionStyles = useCallback(
      (cat: KosherCategory) => {
    ...
    const getCategoryTextStyles = useCallback(
      (cat: KosherCategory) => {
        const isSelected = category === cat.id;
        const baseStyles = [
      ...
      <View style={[styles.container, containerStyle]}>
            ...
            {kosherCategories.map(cat => (
                ...
                style={[
                  getCategoryOptionStyles(cat),
                  {
                    transform: [
                      {
                        scale:
                          categoryAnimations[cat.id]?.interpolate({
                            inputRange: [0, 1],
                            outputRange: [1, 1.02],
                          }) || 1,
                      },
              ...
              >
                <TouchableOpacity
                  ...
                  accessibilityHint={
                    category === cat.id ? 'Selected' : 'Tap to select'
                  }
                >
                  {/* Icon */}
                  <Text
                  ...
                  {/* Label */}
                  <Text style={getCategoryTextStyles(cat)}>{cat.label}</Text>
                  ...
                  {!compact && cat.requirements && (
                    ...
                    >
                      <Text style={styles.requirementsToggleText}>
                        {showRequirements === cat.id ? 'Hide' : 'View'}{' '}
                        Requirements
                      </Text>
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.8}
            accessibilityRole="button"
            accessibilityLabel={`Select certifying agency. Current: ${
              agency || 'None selected'
            }`}
          >
