...
const trackFavoriteEvent = (event: string, data?: any) => {
  try {
    ...
    warnLog('Failed to track analytics event:', error);
  }
};
...
export const useFavorites = (): UseFavoritesReturn => {
  ...
  const addToFavorites = useCallback(
    async (entityId: string): Promise<boolean> => {
      try {
        ...
        errorLog('Error adding to favorites:', err);
  ...
  const removeFromFavorites = useCallback(
    async (entityId: string): Promise<boolean> => {
      try {
        ...
        errorLog('Error removing from favorites:', err);
  ...
  const toggleFavorite = useCallback(
    async (entityId: string, entityData?: any): Promise<boolean> => {
      try {
        setError(null);
        ...
        if (response.success) {
          ...
          if (isFavorited) {
            ...
            // Remove from local favorites list
            setFavorites(prev =>
              prev.filter(fav => fav.entity_id !== entityId),
            );
          }
        ...
        errorLog('Error toggling favorite:', err);
        setError('Failed to toggle favorite');
        return false;
      }
    },
  ...
  const checkFavoriteStatus = useCallback(
    async (entityId: string): Promise<boolean> => {
      try {
        ...
        errorLog('Error checking favorite status:', err);
        return false;
      }
    },
    [],
