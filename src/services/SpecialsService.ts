...
class SpecialsService {
  ...
  async getActiveSpecials(params?: {
    ...
    try {
      ...
      const queryString = queryParams.toString();
      const endpoint = `/specials/active${
        queryString ? `?${queryString}` : ''
      }`;

      const response = await this.request<{ specials: ActiveSpecial[] }>(
        endpoint,
      );
      ...
      console.log(
        'Specials endpoint error, falling back to simple endpoint...',
        error,
      );
      return this.getSimpleSpecials();
    }
  ...
  async getSimpleSpecials(): Promise<
    ApiResponse<{ specials: ActiveSpecial[] }>
  > {
    try {
