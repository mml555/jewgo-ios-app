import {
  BusinessHoursData,
  DayHours,
} from '../components/BusinessHoursSelector';
    ...
    validate: (hours: BusinessHoursData): ValidationResult => {
      ...
      if (openDays.length === 0) {
        return {
          ...
          warnings: {},
          suggestions: [
            'Consider setting standard business hours for weekdays',
          ],
        };
      }
      ...
      Object.values(hours).forEach(dayHours => {
        if (
          ...
          if (closeMinutes <= openMinutes) {
            errors[dayHours.day] =
              'Closing time must be after opening time, or enable "Next Day"';
          }
        }
      });
      ...
      Object.values(hours).forEach(dayHours => {
        if (dayHours.isOpen && dayHours.openTime && dayHours.closeTime) {
          ...
          if (openMinutes < 300) {
            ...
            warnings[
              dayHours.day
            ] = `Very early opening time: ${formatTimeForDisplay(
              dayHours.openTime,
            )}`;
          }

          // Check for very late closing (after 2 AM next day)
          if (dayHours.isNextDay && closeMinutes > 120) {
            // 2:00 AM
            warnings[
              dayHours.day
            ] = `Very late closing time: ${formatTimeForDisplay(
              dayHours.closeTime,
            )} next day`;
          }
          ...
          if (totalMinutes > 960) {
            // 16 hours
            warnings[dayHours.day] = `Very long operating hours: ${Math.floor(
              totalMinutes / 60,
            )} hours`;
            suggestions.push(
              'Consider if such long hours are sustainable for your business',
            );
          }

          // Check for very short hours (less than 2 hours)
          if (!dayHours.isNextDay && totalMinutes < 120) {
            // 2 hours
            warnings[dayHours.day] = `Very short operating hours: ${Math.floor(
              totalMinutes / 60,
            )} hours`;
          }
        }
  ...
  // Rule 6: Consistent weekend/weekday patterns (suggestions only)
  {
    ...
    validate: (hours: BusinessHoursData): ValidationResult => {
      ...
      if (weekdayHours.length > 1) {
        ...
        const inconsistentWeekdays = weekdayHours.some(
          day =>
      ...
      if (weekendHours.length > 1) {
        ...
        const inconsistentWeekends = weekendHours.some(
          day =>
...
export const getBusinessHoursErrors = (
  hours: BusinessHoursData,
): { [day: string]: string } => {
  return defaultValidationEngine.getErrors(hours);
};

export const getBusinessHoursSuggestions = (
  hours: BusinessHoursData,
): string[] => {
